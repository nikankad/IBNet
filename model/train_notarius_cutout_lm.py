...
def _generate_run_id(aug_label=""):
...
def _resolve_checkpoint_dir(checkpoint_dir):
...
def _build_dataloaders(train_ds, val_ds, test_ds, batch_size, num_workers, rank, world_size):
    ...
    if world_size > 1:
        train_sampler = DistributedBucketBatchSampler(
            train_lengths, batch_size=per_device_batch_size,
    ...
    train_loader = DataLoader(train_ds, batch_sampler=train_sampler,
                              collate_fn=collate_fn_cutout, **_loader_kwargs(num_workers))
    ...
    if _is_main_process(rank):
        ...
        val_loader = DataLoader(val_ds, batch_sampler=val_sampler,
                                collate_fn=collate_fn_test, **_loader_kwargs(num_workers))
...
def _build_inference_payload(model, R, C, expand, epoch, best_val_loss, best_val_wer, run_id=None):
    return {
        ...
        "config": {"R": R, "C": C, "expand": expand, "n_mels": 64, "n_classes": 29},
    }


def _reduce_train_metrics(total_loss, total_word_errors, total_ref_words, num_batches, device):
    metrics = torch.tensor([total_loss, total_word_errors, total_ref_words, num_batches],
...
def train_model(
    R=3, C=192, expand=2, num_epochs=50, warmup_epochs=2, lr=0.005,
    output_base="outputs/notarius", save_every=10, resume_from=None,
    batch_size=180, num_workers=16, compile_model=True, run_id=None,
    augmentation=None, arpa_path=DEFAULT_ARPA, beam_width=100,
    lm_alpha=0.5, lm_beta=1.5,
):
    ...
    try:
        ...
        optimizer = NovoGrad(train_model_ref.parameters(), lr=lr, betas=(0.95, 0.5), weight_decay=0.001)
        criterion = nn.CTCLoss(blank=28, zero_infinity=True)

        steps_per_epoch = len(train_loader)
        warmup_steps = warmup_epochs * steps_per_epoch
        cosine_steps = num_epochs * steps_per_epoch - warmup_steps
        ...
        if is_main:
            ...
            write_training_config(
                model=base_model, checkpoint_dir=run_dir, R=R, C=C, expand=expand,
                n_mels=64, n_classes=29, num_epochs=num_epochs, warmup_epochs=warmup_epochs,
                lr=lr, batch_size=batch_size, optimizer_name="NovoGrad(betas=(0.95,0.5), wd=0.001)",
        ...
        train_losses, val_losses, train_wers, val_wers = [], [], [], []
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for batch_idx, (inputs, targets, input_lengths, target_lengths) in enumerate(train_loader):
                ...
                if is_main and (batch_number % log_interval == 0 or batch_number == len(train_loader)):
                    ...
                    eta_seconds = (elapsed / batch_number) * (len(train_loader) - batch_number)
            ...
            reduced = _reduce_train_metrics(
                total_train_loss, total_train_word_errors, total_train_ref_words, len(train_loader), device)
            avg_train_loss = reduced[0] / max(1.0, reduced[3])
            avg_train_wer = (reduced[1] / max(1.0, reduced[2])) * 100
            ...
            if is_main:
                ...
                log_epoch(
                    csv_path=log_csv, epoch=epoch + 1,
                ...
                checkpoint_payload = {
                    ...
                    "train_wers": train_wers, "val_wers": val_wers,
                    "best_val_loss": best_val_loss, "best_val_wer": best_val_wer,
                    "config": {"R": R, "C": C, "expand": expand, "n_mels": 64, "n_classes": 29},
                }
        ...
        if is_main:
            ...
            torch.save(_build_inference_payload(
                base_model, R, C, expand, num_epochs - 1, best_val_loss, best_val_wer, run_id), final_model_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default="outputs/notarius")
    ...
    train_model(
        R=args.R, C=args.C, expand=args.expand, num_epochs=args.epochs,
        warmup_epochs=args.warmup, lr=args.lr, output_base=args.output_dir,
        save_every=args.save_every, resume_from=args.resume,
        batch_size=args.batch_size, num_workers=args.num_workers,
        compile_model=not args.no_compile,
        run_id=_generate_run_id("cutout-lm"),
        augmentation={"speed_perturb": False, "spec_augment": False, "spec_cutout": True},
        arpa_path=args.arpa, beam_width=args.beam_width,
        lm_alpha=args.lm_alpha, lm_beta=args.lm_beta,
    )